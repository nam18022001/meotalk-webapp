...
const handleClickCall = async ({ chatRoomId, userInfo, currentUser, setPressCall }: VideoCallProps) => {
  ...
  if (userInfo.length > 1) {
    ...
    if (!parnerInCall) {
      ...
      if (getDocChats.empty) {
        addFirstMessage({ collectChat, currentUser, data: 'Cuộc gọi nhỡ', call: true });
      } else {
        const dataLast = await getlastMessage({ collectChat });

        addMessage({ collectChat, currentUser, data: 'Cuộc gọi nhỡ', call: true, dataLast });
      }
