  ...
  return !loadingConversation ? (
    <Fragment>
      ...
      >
          ...
          <div className={`${own ? 'justify-end' : ''} w-full flex items-center`}>
            {!own && (
              <img
                ...
                src={isGroup ? photoSender : seenImg}
                alt={'avatar'}
              />
            )}
            <Tippy
        ...
        {isGroup && (
          <div className="min-w-[20px] max-w-[300px] h-full flex items-end mt-[2px]">
            {seen
              ? seenGroup.length > 0 &&
                seenGroup.map(
                  (seen, index) =>
                    seen.seenBy !== currentUser.email && (
                      <img
                        key={index}
                        className="avatar-seen-message-conversation ml-[1px] w-[14px] h-[14px] xs:w-[10px] xs:h-[10px]"
                        src={seen.photoURL}
                        alt={'seen'}
                      />
                    ),
              ...
              : !seen &&
                seenGroup.length === 0 &&
                own && <BsFillCheckCircleFill className="seen-icon-message-conversation" />}
          </div>
        )}
